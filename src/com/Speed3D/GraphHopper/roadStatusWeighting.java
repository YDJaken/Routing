...
public class roadStatusWeighting extends AbstractWeighting {
	...
	public double calcWeight(EdgeIteratorState edgeState, boolean reverse, int prevOrNextEdgeId) {
		double test = 1.0;
		try {
			test = Double.parseDouble(new String(HttpRequestUtil.getRoadStatus(GraphHopperInstance.roadStatusURL,WebHelper.toGeojson(edgeState.fetchWayGeometry(3), false)), "UTF-8"));
	...
	public long calcMillis(EdgeIteratorState edgeState, boolean reverse, int prevOrNextEdgeId) {
		if(flagEncoder instanceof DataFlagEncoder) {
	        ...
	        double maxspeed = ((DataFlagEncoder)flagEncoder).getMaxspeed(edgeState, ((DataFlagEncoder)flagEncoder).getAccessType("motor_vehicle"), reverse);
	        ...
	        boolean unfavoredEdge = edgeState.getBool(EdgeIteratorState.K_UNFAVORED_EDGE, false);
	        if (unfavoredEdge)
	            timeInMillis += Math.round(this.map.getDouble(Routing.HEADING_PENALTY, Routing.DEFAULT_HEADING_PENALTY) *1000);
